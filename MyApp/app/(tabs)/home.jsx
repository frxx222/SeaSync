...
const App = () => {
  ...
  const handleSaveModal = async () => {
    ...
    try {
      const response = await axios.post("http://192.168.1.10:4000/api/market-data", formData);
  ...
  const listData = () => {
    axios
      .get("http://192.168.1.10:4000/api/data")
      .then((response) => {
  ...
  const handleDelete = async (id) => {
    setIsDeleting(true);
    try {
      await axios.delete(`http://192.168.1.10:4000/api/market-data/${id}`);
      ...
      <Modal visible={modalData}>
        <SafeAreaView>
          ...
          <View style={{ paddingHorizontal: 10, marginTop: 20 }}>
            ...
            />
            <Text>Source</Text>
            <View style={[styles.txtInput, { padding: 0 }]}>
              <Picker
                selectedValue={source}
                onValueChange={(itemValue) => setSource(itemValue)}
              >
                <Picker.Item label="Select Source" value="" />
                <Picker.Item label="Boac" value="Boac" />
                <Picker.Item label="Mogpog" value="Mogpog" />
                <Picker.Item label="Gasan" value="Gasan" />
                <Picker.Item label="Buenavista" value="Buenavista" />
                <Picker.Item label="Sta Cruz" value="Sta Cruz" />
                <Picker.Item label="Torrijos" value="Torrijos" />
              </Picker>
            </View>
            <Text>Price</Text>
            <TextInput
